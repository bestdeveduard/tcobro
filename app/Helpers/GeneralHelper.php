        ...
        if ($loan->override_interest == 1) {
            ...
            if ($loan->repayment_cycle == 'annually') {
                ...
                if ($loan->interest_period == 'day') {
                    $interest = $loan->interest_rate * 360;
                }
            }
            if ($loan->repayment_cycle == 'semi_annually') {
                ...
                if ($loan->interest_period == 'day') {
                    $interest = $loan->interest_rate * 182;
                }
            }
            if ($loan->repayment_cycle == 'quarterly') {
                ...
                if ($loan->interest_period == 'day') {
                    $interest = $loan->interest_rate * 91;
                }
            }
...
//determinar el pago mensual mediante amortización
    ...
    //            $period = ceil($loan->loan_duration * 360);
    //        }
    //    }
    //    if ($loan->repayment_cycle == 'semi_annually') {
    //        if ($loan->loan_duration_type == 'year') {
    ...
    //        }
    //        if ($loan->loan_duration_type == 'day') {
    //            $period = ceil($loan->loan_duration * 182);
    //        }
    //    }
    //    if ($loan->repayment_cycle == 'quarterly') {
    //        if ($loan->loan_duration_type == 'year') {
    ...
    //        }
    //        if ($loan->loan_duration_type == 'day') {
    //            $period = ceil($loan->loan_duration * 30);
    //        }
    //    }
    //
    //    if ($loan->repayment_cycle == 'monthly') {
    ...
    public static function loan_paid_item($id, $item = 'principal', $date = '')
    {
        ...
        if (!empty($loan->loan_product)) {
            ...
            foreach ($schedules as $schedule) {
                ...
                //Aplicar el resto a la directora
                $principal = $principal + $payments;
            }
        }
        if ($item == 'principal') {
    ...
    {
        if (empty($start_date)) {
            return Expense::where('branch_id', Sentinel::getUser()->business_id)->sum('amount');
        } else {
            return Expense::where('branch_id', Sentinel::getUser()->business_id)->whereBetween('date',
    ...
    public static function total_payroll($start_date = '', $end_date = '')
    {
        if (empty($start_date)) {
            $payroll = 0;
            foreach (Payroll::where('branch_id', Sentinel::getUser()->business_id)->get() as $key) {
                $payroll = $payroll + GeneralHelper::single_payroll_total_pay($key->id);
            }
            return $payroll;
        } else {
            $payroll = 0;
            foreach (Payroll::where('branch_id', Sentinel::getUser()->business_id)->whereBetween('date',
    ...
    {
        if (empty($start_date)) {
            return OtherIncome::where('branch_id', Sentinel::getUser()->business_id)->sum('amount');
        } else {
            return OtherIncome::where('branch_id', Sentinel::getUser()->business_id)->whereBetween('date',
                [$start_date, $end_date])->sum('amount');
    ...
    {
        if (empty($start_date)) {
            return SavingTransaction::where('branch_id', Sentinel::getUser()->business_id)->where('type',
    ...
    {
        if (empty($start_date)) {
            return SavingTransaction::where('branch_id', Sentinel::getUser()->business_id)->where('type', 'deposit')->where('reversed',0)->sum('credit');
        } else {
            return SavingTransaction::where('branch_id', Sentinel::getUser()->business_id)->where('type',
    ...
    {
        if (empty($start_date)) {
            return Capital::where('branch_id', Sentinel::getUser()->business_id)->where('type',
                    'deposit')->sum('amount') - Capital::where('branch_id', Sentinel::getUser()->business_id)->where('type',
    ...
    public static function loans_product_total_paid_item($id, $item, $start_date = '', $end_date = '')
    {
        if (empty($start_date)) {
            $amount = 0;
            foreach (Loan::where('loan_product_id', $id)->where('branch_id', Sentinel::getUser()->business_id)->whereIn('status',
                ['disbursed', 'closed', 'written_off'])->get() as $key) {
                $amount = $amount + GeneralHelper::loan_terms_paid_item($key->id, $item);
            }
            return $amount;
    ...
    public static function loans_borrower_total_paid_item($id, $item, $start_date = '', $end_date = '')
    {
        if (empty($start_date)) {
            $amount = 0;
            foreach (Loan::where('borrower_id', $id)->where('branch_id', Sentinel::getUser()->business_id)->whereIn('status',
                ['disbursed', 'closed', 'written_off'])->get() as $key) {
                $amount = $amount + GeneralHelper::loan_terms_paid_item($key->id, $item);
            }
            return $amount;
    ...
    public static function loans_total_default($start_date = '', $end_date = '')
    {
        if (empty($start_date)) {
            $principal = 0;
            foreach (Loan::where('branch_id', Sentinel::getUser()->business_id)->where('status', 'written_off')->get() as $key) {
                $principal = $principal + ($key->principal - GeneralHelper::loan_total_paid($key->id));
            }
            return $principal;
        } else {
            $principal = 0;
            foreach (Loan::where('branch_id', Sentinel::getUser()->business_id)->where('status',
    ...
    public static function loans_total_due($start_date = '', $end_date = '')
    {
        if (empty($start_date)) {
            $due = 0;
            foreach (Loan::where('branch_id', Sentinel::getUser()->business_id)->whereIn('status',
                ['disbursed', 'written_off'])->get() as $key) {
            ...
            $due = 0;
            foreach (Loan::where('branch_id', Sentinel::getUser()->business_id)->whereIn('status',
                ['disbursed', 'written_off'])->whereBetween('release_date',
                [$start_date, $end_date])->get() as $key) {
                $due = $due + GeneralHelper::loan_total_due_amount($key->id);
            }
            return $due;
    ...
    public static function loans_count($start_date = '', $end_date = '')
    {
        if (empty($start_date)) {
            $due = 0;
            $due = $due + Loan::where('branch_id', Sentinel::getUser()->business_id)->whereIn('status',
    ...
    public static function audit_trail($notes)
    {
        ...
        $audit_trail->branch_id = Sentinel::getUser()->business_id;
        $audit_trail->save();
    ...
    public static function bank_account_balance($id)
    {
        ...
        return Capital::where('bank_account_id', $id)->where('branch_id', Sentinel::getUser()->business_id)->where('type',
                'deposit')->sum('amount') - Capital::where('bank_account_id', $id)->where('branch_id',
                Sentinel::getUser()->business_id)->where('type',
                'withdrawal')->sum('amount');
    }
    ...
    public static function send_whatsapp($to, $msg) {
        $api_instance = Setting::where('setting_key','whatsapp_api_url')->first()->setting_value;
        $api_token = Setting::where('setting_key','whatsapp_token')->first()->setting_value;
        ...
        $token = $api_token;//'8uv8mvjfnyk61w6w';
        $instanceId = $api_instance;//'181679';
    ...
    public static function check_ins_total_amount($start_date = '', $end_date = '')
    {
        if (empty($start_date)) {
            $due = 0;
            foreach (ProductCheckin::where('branch_id', Sentinel::getUser()->business_id)->get() as $key) {
                $due = $due + GeneralHelper::check_in_total_amount($key->id);
            }
            return $due;
        } else {
            $due = 0;
            foreach (ProductCheckin::where('branch_id', Sentinel::getUser()->business_id)->whereBetween('date',
                [$start_date, $end_date])->get() as $key) {
                $due = $due + GeneralHelper::check_in_total_amount($key->id);
            }
            return $due;
    ...
    public static function check_outs_total_amount($start_date = '', $end_date = '')
    {
        if (empty($start_date)) {
            $due = 0;
            foreach (ProductCheckout::where('branch_id', Sentinel::getUser()->business_id)->get() as $key) {
                $due = $due + GeneralHelper::check_in_total_amount($key->id);
            }
            return $due;
        } else {
            $due = 0;
            foreach (ProductCheckout::where('branch_id', Sentinel::getUser()->business_id)->whereBetween('date',
                [$start_date, $end_date])->get() as $key) {
                if ($key->type == 'cash') {
                    $due = $due + GeneralHelper::check_in_total_amount($key->id);
                } else {
    ...
    public static function loan_allocate_payment($loan_transaction)
    {
        ...
        if (!empty($loan->loan_product)) {
            // encuentra todos los pagos hasta esta fecha
            // resta este pago actual
            ...
            foreach ($loan->schedules as $schedule) {
                ...
                //estos horarios han sido cubiertos
                    } else {
                //$schedules have not yet been covered
                        if ($payments > 0) {
                //intente asignar el pago restante a los elementos respectivos
                            $repayment_order = unserialize($loan->loan_product->repayment_order);
                            foreach ($repayment_order as $order) {
                                if ($order == 'interest') {
    ...
    public static function loans_paid_items($start_date = '', $end_date = '')
    {
        ...
        if (empty($start_date)) {
            $principal = $principal + JournalEntry::where('transaction_type',
                    ...
                    0)->where('branch_id', Sentinel::getUser()->business_id)->sum('credit');
            ...
            $fees = $fees + JournalEntry::where('transaction_type', 'repayment')->where('transaction_sub_type',
                    'repayment_fees')->where('reversed', 0)->where('branch_id', Sentinel::getUser()->business_id)->sum('credit');
            $penalty = $penalty + JournalEntry::where('transaction_type', 'repayment')->where('transaction_sub_type',
                    'repayment_penalty')->where('reversed', 0)->where('branch_id', Sentinel::getUser()->business_id)->sum('credit');
            $over_payments = $over_payments + JournalEntry::where('transaction_type',
                    'repayment')->where('transaction_sub_type',
                    'overpayment')->where('reversed', 0)->where('branch_id', Sentinel::getUser()->business_id)->sum('credit');
        } else {

            $principal = $principal + JournalEntry::where('transaction_type',
                    'repayment')->where('transaction_sub_type', 'repayment_principal')->where('reversed',
                    0)->whereBetween('date',
                    [$start_date, $end_date])->where('branch_id', Sentinel::getUser()->business_id)->sum('credit');
            $interest = $interest + JournalEntry::where('transaction_type', 'repayment')->where('transaction_sub_type',
