...
//        $user_id = $request->user_id;
        ...
        if (!empty($end_date)) {
            ...
            $data = [];
            array_push($data, [
                trans_choice('general.loan', 1) . ' ' . trans_choice('general.disbursed',
                    2)
            ]);
            array_push($data, [
                trans_choice('general.id', 1),
                trans_choice("Nombre cliente", 1),
                trans_choice("Ruta", 1),
                trans_choice("Fecha desembolso", 1),
                trans_choice("Fecha vencimiento", 1),
                trans_choice("Capital", 1),
                trans_choice("Interes", 1),
                trans_choice("Ajustes", 2),
                trans_choice("Mora", 1),
                trans_choice("Balance total", 1),
                trans_choice("Balance pagado", 2),
                trans_choice("Balance pendiente", 1),
            ]);
            $total_outstanding = 0;
            $total_due = 0;
            $total_payments = 0;
            ...
            Excel::create(trans_choice('general.loan', 1) . ' ' . trans_choice('general.disbursed',
                    2),
                function ($excel) use ($data) {
                    $excel->sheet('Sheet', function ($sheet) use ($data) {
                        $sheet->fromArray($data, null, 'A1', false, false);
