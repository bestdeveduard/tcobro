...
class CronController extends Controller
{
    ...
    public function index()
    {
        if (Setting::where('setting_key', 'enable_cron')->first()->setting_value == 0) {
            ...
            if (Setting::where('setting_key', 'auto_repayment_email_reminder')->first()->setting_value == 1) {
                ...
                foreach ($schedules as $schedule) {
                    ...
                    if (!empty($schedule->borrower->email)) {
                        ...
                        $body = str_replace('{loanPayments}', GeneralHelper::loan_total_paid($schedule->loan_id), $body);
                        $body = str_replace('{loanDue}',
                            round(GeneralHelper::loan_total_due_amount($schedule->loan_id), 2), $body);
                        $body = str_replace('{loanBalance}',
                            round((GeneralHelper::loan_total_due_amount($schedule->loan_id) - GeneralHelper::loan_total_paid($schedule->loan_id)),
                                2), $body);
            ...
            if (Setting::where('setting_key', 'whatsapp_enabled')->first()->setting_value == 1) {
                $days = Setting::where('setting_key', 'auto_repayment_days')->first()->setting_value;
                $due_date = date_format(date_add(date_create(date("Y-m-d")),
                    date_interval_create_from_date_string($days . ' days')),
                    'Y-m-d');
                ...
                foreach ($schedules as $schedule) {
                    //check if borrower has email
                    if (!empty($schedule->borrower)) {
                        $borrower = $schedule->borrower;
                        
                        $body = Setting::where('setting_key',
                            'loan_payment_reminder_whatsapp_template')->first()->setting_value;
                        
                        $body = str_replace('{borrowerFirstName}', $borrower->first_name, $body);                        
                        $body = str_replace('{loanNumber}', $schedule->loan_id, $body);
                        $body = str_replace('{paymentAmount}', round(($schedule->principal + $schedule->interest + $schedule->fees + $schedule->penalty), 2), $body);
                        $body = str_replace('{paymentDate}', $schedule->due_date, $body);
                        
                        if (!empty($borrower->mobile)) {
                            GeneralHelper::send_whatsapp($borrower->mobile, $body);
                        }
                    }
                }
            }
            ...
            if (Setting::where('setting_key', 'auto_overdue_repayment_email_reminder')->first()->setting_value == 1) {
                ...
                foreach ($schedules as $schedule) {
                    ...
                    if (!empty($schedule->borrower->email)) {
                        ...
                        $payments = LoanRepayment::where('loan_id', $schedule->loan_id)->where('due_date',
            ...
            if (Setting::where('setting_key', 'whatsapp_enabled')->first()->setting_value == 1) {
                ...
                foreach ($schedules as $schedule) {
                    ...
                    if (!empty($schedule->borrower)) {
                        ...
                        if ($payments == 0) {
                            $body = Setting::where('setting_key',
                                'missed_payment_whatsapp_template')->first()->setting_value;
            ...
            if (Setting::where('setting_key', 'whatsapp_enabled')->first()->setting_value == 1) {
                ...
                foreach ($loans as $loan) {
                    ...
                    if (!empty($loan->borrower)) {
                        $borrower = $loan->borrower;
                        $body = Setting::where('setting_key',
                            'loan_overdue_whatsapp_template')->first()->setting_value;
            ...
            $loans = Loan::where('status', 'disbursed')->where('penalty_status', 1)->get();
                ...
                if (!empty($loan->loan_product)) {
                    if ($loan->loan_product->enable_late_repayment_penalty == 1) {
                        ...
                        foreach ($schedules as $schedule) {
                            ...
                            if ($date < date("Y-m-d") && $outstanding > 0) {
                                if (GeneralHelper::loan_total_due_period($loan->id,
                                    ...
                                    if ($loan->loan_product->late_repayment_penalty_type == "fixed") {
                                        $sch->penalty = $loan->loan_product->late_repayment_penalty_amount;
                                    } else {
                                        ...
                                        // Interest method 3 - Balance Principal + Interest + Fee
                                        if ($loan->loan_product->late_repayment_penalty_calculate == 'overdue_principal_interest_fees') {
                                        
                                        $loan_total_pri_3 = GeneralHelper::loan_total_principal($loan->id,$schedule->due_date);
                                        $loan_total_int_3 = GeneralHelper::loan_total_interest($loan->id,$schedule->due_date);
                                        $loan_total_fee_3 = GeneralHelper::loan_total_fees($loan->id,$schedule->due_date);
                                         $total_balance_3 = $loan_total_pri_3 + $loan_total_int_3 + $loan_total_fee_3;
                                     //   $loan_total_pri_paid_3 = GeneralHelper::loan_paid_item($loan->id, 'principal', $schedule->due_date);
                                     //   $loan_total_int_paid_3 = GeneralHelper::loan_paid_item($loan->id, 'interest', $schedule->due_date);
                                     //   $loan_total_fee_paid_3 = GeneralHelper::loan_paid_item($loan->id, 'fees', $schedule->due_date);
                                     //    $total_balance_paid_3 = $loan_total_pri_paid_3 + $loan_total_int_paid_3 + $loan_total_fee_paid_3;
                                        $totalpayment = \App\Models\LoanTransaction::where('loan_id',$loan->id)->where('transaction_type','repayment')->where('reversed', 0)->sum('credit');;
                                        $principal = $total_balance_3 - $totalpayment;
                                        $abc_principal = abs($principal);
                                  
                                        $sch->penalty = (($loan->loan_product->late_repayment_penalty_amount / 100) * $abc_principal);
                                            
                                        }
                                        
                                        if ($loan->loan_product->late_repayment_penalty_calculate == 'total_overdue') {
                                            $principal = (GeneralHelper::loan_total_due_amount($loan->id, $schedule->due_date)
                                            - GeneralHelper::loan_total_paid($loan->id, $schedule->due_date));
                                            $sch->penalty = (($loan->loan_product->late_repayment_penalty_amount / 100) * $principal);
                                        }
            ...
            foreach ($loans as $loan) {
                ...
                if (!empty($product)) {
                    //check for charges
                    foreach (LoanProductCharge::where('loan_product_id', $product->id)->get() as $tkey) {
                        if (!empty($tkey->charge)) {
                            $date = date_format(date_sub(date_create(date("Y-m-d")),
                                date_interval_create_from_date_string($product->late_repayment_penalty_grace_period.' days')), 'Y-m-d');
                            //missed penalty charge
                            if ($tkey->charge->charge_type == "overdue_installment_fee") {
                                $schedule = LoanSchedule::where('loan_id', $loan->id)->where('due_date', $date)->first();
