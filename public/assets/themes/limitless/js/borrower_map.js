...
function initialize() {
  ...
  if (lat == 'none' || lng == 'none') {
    navigator.geolocation.getCurrentPosition(function(position) {
      ...
      marker.setMap(map);
      $('#geolocation').attr("value", position.coords.latitude + "," + position.coords.longitude);
    });
  } else {
    marker = new google.maps.Marker({
      position: latlng,
    ...
    marker.setMap(map);
    $('#geolocation').attr("value", lat + "," + lng);
  }
  
  google.maps.event.addListener(map, 'click', function(e) {
    marker.setPosition(e.latLng);
    $('#geolocation').attr("value", e.latLng.lat() + "," + e.latLng.lng());
  });	
}